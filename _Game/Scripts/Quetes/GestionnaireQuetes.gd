class_name GestionnaireQuetes extends Node

signal quete_demarree(quete: QueteSO)
signal etape_validee(quete: QueteSO, indice: int)
signal quete_terminee(quete: QueteSO)
...
func _terminer_quete(quete: QueteSO) -> void:
	...
	if quete.recompense and inventaire:
